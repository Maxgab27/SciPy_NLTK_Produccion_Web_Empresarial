    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <section>
        ...
        {met?.coeficiente_variacion !== undefined && (
          <div style={{ ...card, marginTop: '1rem', fontSize: '0.85rem', color: 'var(--text-primary)' }}>
      ...
      <section>
        ...
        <div style={card}>
          <svg width="100%" viewBox={`0 0 ${puntos.length * 80} ${BAR_H + 40}`} style={{ overflow: 'visible' }}>
            {puntos.map((p, i) => {
              ...
              const color = p.tipo === 'real' ? 'var(--primary)' : 'var(--text-secondary)';
              return (
                <g key={p.mes}>
                  <rect x={x} y={y} width={40} height={barH} fill={color} rx={3} />
                  <text x={x + 20} y={y - 4} textAnchor="middle" fontSize="10" fill="var(--text-secondary)">{(p.ventas / 1000).toFixed(1)}k</text>
                  <text x={x + 20} y={BAR_H + 25} textAnchor="middle" fontSize="11" fill={p.tipo === 'estimado' ? 'var(--text-secondary)' : 'var(--text-primary)'} fontWeight={p.tipo === 'real' ? 600 : 400}>{p.mes}</text>
                </g>
              );
            })}
          </svg>
          <div style={{ display: 'flex', gap: '1.5rem', marginTop: '0.75rem', fontSize: '0.78rem', color: 'var(--text-secondary)' }}>
            <span><span style={{ display: 'inline-block', width: 12, height: 12, backgroundColor: 'var(--primary)', borderRadius: 2, marginRight: 4, verticalAlign: 'middle' }} />Dato real</span>
            <span><span style={{ display: 'inline-block', width: 12, height: 12, backgroundColor: 'var(--text-secondary)', borderRadius: 2, marginRight: 4, verticalAlign: 'middle' }} />Estimado (interp1d)</span>
          </div>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem', marginTop: '1rem' }}>
          ...
          <tbody>
            {puntos.map((p) => (
              <tr key={p.mes} style={{ borderBottom: '1px solid var(--border-color)' }}>
                <td style={td}><strong>{p.mes}</strong></td>
                <td style={td}>S/ {p.ventas.toLocaleString()}</td>
                <td style={td}>
                  <span style={{ fontSize: '0.72rem', padding: '0.15rem 0.5rem', borderRadius: '3px', backgroundColor: p.tipo === 'real' ? 'var(--success-bg)' : 'var(--surface-muted)', color: p.tipo === 'real' ? 'var(--success)' : 'var(--text-secondary)', border: '1px solid', borderColor: p.tipo === 'real' ? 'var(--success-border)' : 'var(--border-color)' }}>
                    {p.tipo}
                  </span>
                </td>
              </tr>
...
function StatCard({ label, value, code }: { label: string; value: string; code?: string }) {
  return (
    <div style={card}>
      <span style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', display: 'block' }}>{label}</span>
      <span style={{ fontSize: '1.4rem', fontWeight: 700, color: 'var(--text-primary)', display: 'block', margin: '0.2rem 0' }}>{value}</span>
      {code && <code style={{ fontSize: '0.7rem', color: 'var(--primary)', fontFamily: 'monospace' }}>{code}</code>}
    </div>
  );
}

function StatusBadge({ online }: { online: boolean | null }) {
  return <span style={{ fontSize: '0.78rem', padding: '0.3rem 0.7rem', borderRadius: '4px', border: `1px solid ${online ? 'var(--success-border)' : 'var(--border-color)'}`, backgroundColor: online ? 'var(--success-bg)' : 'var(--surface-muted)', color: online ? 'var(--success)' : 'var(--text-secondary)' }}>{online === null ? 'Conectando...' : online ? 'Backend Activo' : 'Demostracion'}</span>;
}
...
const card: React.CSSProperties = { backgroundColor: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '1rem', boxSizing: 'border-box', boxShadow: 'var(--card-shadow)' };
